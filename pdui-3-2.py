...
# Load models
import joblib
clf_3 = joblib.load("updated_updated_model_decline_3.joblib")
clf_5 = joblib.load("updated_updated_model_decline_5.joblib")
...
# Ensure feature columns match training (only use '1_' features and sort them)
feature_cols = [col for col in player_df.columns if col.startswith("1_")]
X = player_df[feature_cols].copy()
X = X.reindex(sorted(X.columns), axis=1)

# Run predictions using the updated models
